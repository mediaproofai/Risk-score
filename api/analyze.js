export default async function handler(req, res) {
    ...
    try {
        ...
        function generateNaturalLanguageReport(riskScore, evidence) {
    if (riskScore < 20) {
        return "Our forensic analysis indicates this media is highly likely authentic. Biological signal analysis and metadata consistency checks passed with no anomalies.";
    }
    
    let report = `Our systems have flagged this media as ${riskScore > 80 ? "CRITICAL" : "SUSPICIOUS"}. `;
    
    if (evidence.some(e => e.includes("Physics"))) {
        report += "Micro-texture analysis reveals statistical anomalies in pixel entropy, suggesting the image was generated by a diffusion model rather than a physical camera sensor. ";
    }
    
    if (evidence.some(e => e.includes("Metadata"))) {
        report += "Furthermore, the file lacks mandatory hardware provenance data (EXIF), which is inconsistent with original camera captures. ";
    }
    
    if (evidence.some(e => e.includes("Audio"))) {
        report += "Spectral analysis detected discontinuities in the noise floor consistent with text-to-speech synthesis. ";
    }
    
    report += "Recommendation: Treat as synthetic content.";
    return report;
}
    }
}
